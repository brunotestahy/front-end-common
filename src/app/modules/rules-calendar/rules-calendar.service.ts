...
export class RulesCalendarService {
  ...
  getRulesCalendar(dateShifted: boolean, viewDate: Date) {
    ...
    for (let i = 0; i < RANGE_DAYS * 2 + 1; i++) {
      rulesCalendar.push(this.getCalendarTask(i, viewDate, dateShifted));
    }
  ...
  getCalendarTask(i: number, viewDate: Date, dateShifted: boolean): RulesCalendar {
    ...
    if (dateShifted && this.isSameDate(thisDate, today) ) {
      type = 'todayShifted';
    }
